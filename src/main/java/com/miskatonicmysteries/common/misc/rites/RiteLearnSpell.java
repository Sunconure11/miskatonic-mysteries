...
public class RiteLearnSpell extends OctagramRite {
    ...
    public void doRitual(TileEntityOctagram octagram, @Nullable EntityPlayer caster) {
        ...
        if (caster != null){
            caster.move(MoverType.SELF, (octagram.getPos().getX() + 0.5F - caster.posX) / 20F, (octagram.getPos().getY() + 0.2F - caster.posY) / 20F,(octagram.getPos().getZ() + 0.5F - caster.posZ) / 20F);
        }
